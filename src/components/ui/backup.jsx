...
export function backup({ currentMaterial,currentMaterialS, isImageActive, setIsImageActive, ...props }) {
  ...
  const handleMaterialChange = (material) => {
    ...
    setIsImageActive(false); // override image
  };
  ...
  const handleImageUpload = (event) => {
    ...
    reader.onload = () => {
      ...
      console.log("Is PNG Transparent?", isPng);
      setImageInfo({ url: reader.result, isTransparent: isPng });
      setIsImageActive(true); // image takes control
...
<mesh geometry={nodes.Cube.geometry}>
  {isImageActive ? (
    ...
    <primitive object={currentMaterial || materials.lotimain} attach="material" />
  )}
</mesh>
      ...
      <group position={[0.004, -0.005, 0.06]} rotation={[Math.PI, 0, Math.PI]}>
        <mesh geometry={nodes.Cube002.geometry} material={currentMaterial || materials.lotimain} />
